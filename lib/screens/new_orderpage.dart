// @dart=2.9
// ignore_for_file: prefer_const_constructors_in_immutables, curly_braces_in_flow_control_structures, prefer_const_constructors, prefer_const_literals_to_create_immutables, sized_box_for_whitespace, avoid_single_cascade_in_expression_statements
...
class NewOrderPageState extends State<NewOrderPage> {
  ...
  Future<void> getCustomerId() async {
    GetVouchers().getVouchers().whenComplete(() => {
          print(User.voucherNumber),
          print(User.orderNumber),
          print(User.returnNumber),
          print(User.cashReceiptNumber)
        });
  ...
  void addtoInvoiceValues() {
    ...
    am = double.parse(am.toStringAsFixed(2));
    ...
    Map<String, dynamic> values = {
      ...
      'ArabicName': "",
      'ItemCount': itemname.length.toString(),
  ...
  Future<void> addtoOrders() async {
    ...
    for (int i = 0; i < itemname.length; i++) {
      Map<String, dynamic> itemValues = {
        ...
        'CESSAmount': "0",
        'Code': codeList[i],
        'DiscAmount': discountAmount[i].toStringAsFixed(2),
        'DiscPercentage': percentages[i],
        'GSTAmount': gstTotal[i],
        'InclusiveRate': "",
        'ItemName': itemname[i],
    ...
    prefs.setInt("ordernumber", int.parse(lastVoucher));
    ...
    if (await DataConnectionChecker().hasConnection) {
      reference
          ...
          .set(values).whenComplete(() => {
      reference
      .child("Vouchers")
          .child(User.vanNo)
          .child("OrderNumber").set(lastVoucher.toString())
      });
  ...
  saveToDb(Map<String, dynamic> finalData) async {
    ContactinfoModel contactinfoModel = ContactinfoModel(
        id: null, userId: finalData['OrderID'].toString(), createdAt: "Order",email: finalData['VoucherDate'].toString());
    await Controller().addData(contactinfoModel).then((value) {
      if (value > 0) {
        EasyLoading.showSuccess(finalData['OrderID']+' Successfully Saved');
        Navigator.pop(context);
      } else {
                                ...
                                Container(
                                  ...
                                  child: DropdownButtonHideUnderline(
                                    child: DropdownButton(
                                      ...
                                      hint: Padding(
                                        padding:
                                            EdgeInsets.only(left: 8.0, top: 5),
                                        child: Text(
                                          unit,
